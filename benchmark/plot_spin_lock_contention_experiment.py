...
def _plot_spinlock_vs_atomic(spinlock, atomic):
    ...
    plt.plot(num_positions, spinlock_contention_time, label="Spinlock contention")
    ...
    filename = "plots/contention_experiment/spinlock_vs_atomic.pdf"
    plt.savefig(filename, dpi=400, bbox_inches="tight")
    os.system(f"pdfcrop {filename} {filename}")
...
def plot_spin_lock_vs_atomic(spin_lock_path, atomic_path):
    ...
    spin_lock_data = _parse_data(spin_lock_raw)
    atomic_data = _parse_data(atomic_raw)

    _plot_spinlock_vs_atomic(spin_lock_data, atomic_data)
