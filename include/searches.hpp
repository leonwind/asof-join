...
namespace {
    inline size_t run_interpolation_step(uint64_t target, uint64_t first_val, uint64_t last_val, size_t n) {
...
namespace Search::Interpolation {
    ...
    inline T* less_equal_than(std::vector<T>& data, uint64_t target) {
        ...
        if (n > 1 && target >= data[0].get_key() && target <= data[n - 1].get_key()) {
            pos = run_interpolation_step(target, data[0].get_key(), data[n - 1].get_key(), n);
        }
        ...
        size_t right = overestimated ? pos + 1 : std::min(bound + 1, n);
        T* result = nullptr;
